...
class TMFG:
    ...
    def _compute_TMFG(self) -> tuple[list[list[int]], list[list[int]], np.ndarray]:
        ...
        while len(self._cliques) < self._N - 3:
            ...
            if len(self._cliques) % 500 == 0:
                print(f"TMFG construction: {len(self._cliques)} vertices inserted")
    ...
    def _max_clique(self) -> list[int]:
        ...
        # Use broadcasting and boolean masking to efficiently compute the scores
        mask = self._W > mean_val  # Create a boolean mask of values above the mean
        v = np.sum(self._W * mask, axis=1)  # Sum rows with the condition applied
        # Select the top-4 scoring vertices
        return list(np.argpartition(v, -4)[-4:][np.argsort(v[-4:])][::-1])
